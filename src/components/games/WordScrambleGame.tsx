  ...
  if (finished) {
    ...
    return (
      <div className="glass rounded-3xl p-8 text-center">
        ...
        <h2 className="text-2xl font-bold">Lieliski!</h2>
        <p className="mt-2 text-muted">{correct} no {words.length} vārdiem pareizi</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between text-sm text-muted">
        <span>Rounds: {index + 1}/{words.length}</span>
        <span>Pareizi: {correct}</span>
      </div>
      ...
      >
        <p className="text-center text-sm text-muted">Ko nozīmē:</p>
        ...
        <div className="mt-6 flex min-h-[3rem] flex-wrap justify-center gap-2">
          ...
          {picked.length === 0 && (
            <span className="text-muted text-sm">Noklikšķiniet uz burtiem...</span>
          )}
        </div>
